# Product Requirements Document v2.0: Prediction Market Analytics Platform
...
- **Marketing Pages (`/`)**: Static Site Generation (SSG)
  ...
  - Fast initial load
- **App Pages (`/app/*`)**: Client-Side Rendering (CSR)
  - Fully interactive, data-driven
  - No server-side data fetching (all data is local)
...
├── lib/
│   ├── db/                   # wa-sqlite setup, migrations
│   ├── parsers/              # PDF parsing logic (versioned)
│   ├── calculations/         # P&L, metrics, aggregations
│   └── utils/
├── tests/
│   ├── unit/
│   ├── integration/
│   └── e2e/
└── public/
    └── sample-data/          # Demo mode fixtures
```
